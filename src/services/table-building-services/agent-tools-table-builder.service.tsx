...
export class AgentToolsTableBuilder<T extends object> extends AbstractTableBuilder<T> {
    public generateBaseTable(data: T[],
        ...
        return TableWrapperService.buildTableWrapperProps<T>(
            ...
            AtBaseTable,
            columnsDefinition,
            tableOptionsOverrides ? tableOptionsOverrides : GetDefaultTableWrapperOptionsConfig()
        );
    }

    public generateTableWithOverrides(data: T[],
                                      columnsDefinition: Array<Partial<TableWrapperColumnConfig<T>>>,
        ...
        return TableWrapperService.buildTableWrapperProps<T>(
            ...
            tableOverridesModel.tableStylingConfig,
            tableOverridesModel.tableCellEditConfig,
            tableOverridesModel.tableKeyboardNavigationConfig,
            tableOverridesModel.tableManipulationConfig,
            tableOverridesModel.tableRowExpansionConfig,
            tableOverridesModel.tableRowSelectionConfig
        );
    }

    public generateTableElement(tableModel: BuiltTableConfig<T>): JSX.Element | BaseError {
        if (!tableModel) {
    ...
    public generateColumnsFromKeyArray(keys: string[]): BaseError | TableWrapperColumnConfig<T>[] {
        if (!keys || keys.length === 0) {
            return new BaseError('Cannot create columns since no keys were specified');
        } else {
            return TableWrapperService.buildTableColumnsDefinitionFromKeyArray(keys);
        }
    }
}
